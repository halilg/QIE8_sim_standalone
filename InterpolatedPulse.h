...
public:
    ...
    {
        if (length_ < 2 || length_ > MaxLen) throw std::invalid_argument(
    ...
    inline InterpolatedPulse(const InterpolatedPulse<Len2>& r)
        : tmin_(r.tmin_), width_(r.width_), length_(r.length_)
    {
        if (length_ > MaxLen) throw std::invalid_argument(
    ...
    {
        if (r.length_ > MaxLen) throw std::invalid_argument(
    ...
    {
        if (len < 2 || len > MaxLen) throw std::invalid_argument(
    ...
    {
        if (newWidth <= 0.0) throw std::invalid_argument(
            "In InterpolatedPulse::setPulseWidth: invalid pulse width");
        width_ = newWidth;
    }

    // Get the pulse value at the given time
    ...
    inline void setIntegral(const double newValue)
    {
        ...
        if (integ == 0.0) throw std::runtime_error(
    ...
    inline void setPeakValue(const double newValue)
    {
        ...
        if (peak == 0.0) throw std::runtime_error(
