...
{
    if (which >= dim_) throw std::invalid_argument(
        "In ConstantStepOdeSolver::getPeakTime: index out of range");
    if (runLen_ < 3) throw std::runtime_error(
...
{
    if (which >= dim_ || idx >= runLen_) throw std::invalid_argument(
...
{
    if (runLen_ < 4) throw std::runtime_error(
                                               ...
                                               const double maxValue)
{
    ...
    if (minValue > maxValue) throw std::invalid_argument(
                                ...
                                const double dt, const unsigned nSteps)
{
    if (!nSteps || !lenInitialConditions)
        return;
    if (!rhs_) throw std::runtime_error(
