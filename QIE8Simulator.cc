                            ...
                            double* TS, const unsigned lenTS)
{
    ...
    if (dt <= 0.0) throw std::invalid_argument(
        "In QIE8Simulator::run: invalid time step");

    if (tstop < dt) throw std::invalid_argument(
        "In QIE8Simulator::run: invalid stopping time");

    if (lenTS && tDigitize < 0.0) throw std::invalid_argument(
    ...
    else
    {
        ...
        if (!(integrateToGetCharge_ && chargeNode_ == 0U))
            throw std::runtime_error("In QIE8Simulator::run: "
    ...
    // Integrate the accumulated charge
    unsigned filled = 0;
    if (lenTS)
    {
        assert(TS);
...
{
    if (!runCount_) throw std::runtime_error(
...
double QIE8Simulator::preampPeakTime() const
{
    ...
    const unsigned preampNode = getRHS().outputNode();
    if (preampNode >= AbsElectronicODERHS::invalidNode) throw std::runtime_error(
    ...
    const double* values, const unsigned sz)
{
    ...
    if (sz != nExpected) throw std::invalid_argument(
...
{
    if (inputGain_ <= 0.0) throw std::invalid_argument(
